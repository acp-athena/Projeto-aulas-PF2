...
import ImovelEntity from "../entities/imovelEntity";
...
export default class ImovelRepository {
    ...
    async listar() {
        ...
        for(let i = 0; i < rows.length; i++) {
            let row = rows[i];
            lista.push(new ImovelEntity(row["imv_id"], row["imv_descricao"], 
                row["imv_endereco"],
    ...
    async obter(id) {
        ...
        for(let i = 0; i < rows.length; i++) {
            let row = rows[i];
            lista.push(new ImovelEntity(row["imv_id"], row["imv_descricao"], row["imv_endereco"],
                row["imv_cep"], row["imv_bairro"], row["imv_cidade"], row["imv_valor"], 
                row["imv_disponivel"]
            ));
        }
